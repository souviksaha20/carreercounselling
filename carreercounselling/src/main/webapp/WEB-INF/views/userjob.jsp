...
<div id="dd-elements" class="clearfix">
	<div class="panel">
    	...
    	<c:forEach var="cc" items="${jobs}">
    		<div class="drag-item" draggable="true" id="${cc.getJob_id()}"> ${cc.getJob_name()}</div>
    	</c:forEach>
  	</div>
  	<div id="drop-target-one" class="panel"><p class="mt0" style="color:white;"><strong>Drop some items</strong></p>
		<c:forEach var="cc" items="${ujlist}">
    		<div class="drag-item" draggable="true" id="${cc.getJob().getJob_id()}"> ${cc.getJob().getJob_name()}</div>
    	</c:forEach>
    	...
    	<c:url var="add" value="/report"></c:url>
