...
<div id="dd-elements" class="clearfix">
	<div class="panel">
    	<p class="mt0"><strong>Drag from here</strong></p>
    	<c:forEach var="cc" items="${strs}">
    		<div class="drag-item" draggable="true" id="${cc.getStr_id()}"> ${cc.getStr_name()}</div>
    	</c:forEach>
  	</div>
  	<div id="drop-target-one" class="panel"><p class="mt0" style="color:white;"><strong>Drop your strengths here</strong></p>
		<c:forEach var="cc" items="${uslist}">
			<div class="drag-item" draggable="true" id="${cc.getStrength().getStr_id()}"> ${cc.getStrength().getStr_name()}</div>
    	</c:forEach>
    	...
    	<c:url var="add" value="/userJob"></c:url>
